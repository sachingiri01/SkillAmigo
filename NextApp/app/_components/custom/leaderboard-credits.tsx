...
function getRankIcon(rank: number) {
  switch (rank) {
    case 1:
      return <Crown className="w-3 sm:w-4 md:w-5 h-3 sm:h-4 md:h-5 text-jet-stream-300 drop-shadow-lg" />
    case 2:
      return <Award className="w-3 sm:w-4 md:w-5 h-3 sm:h-4 md:h-5 text-jet-stream-300 drop-shadow-lg" />
    case 3:
      return <Star className="w-3 sm:w-4 md:w-5 h-3 sm:h-4 md:h-5 text-accent drop-shadow-lg" />
    default:
      return <span className="text-xs sm:text-sm font-bold text-gray-300">#{rank}</span>
  }
}
...
function getTrendIcon(trend: string) {
  switch (trend) {
    case "up":
      return <TrendingUp className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-green-400 drop-shadow-sm" />
    case "down":
      return <TrendingUp className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-red-400 rotate-180 drop-shadow-sm" />
    default:
      return null
  }
}
...
export default function LeaderboardCredits() {
  ...
  return (
    <section 
      ref={sectionRef}
      className="bg-jet-stream-950 py-8 sm:py-12 md:py-16 lg:py-20 xl:py-24 2xl:py-32 overflow-hidden relative"
    >
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden">
        <motion.div
        ...
        >
          ...
          <motion.div
            ...
            className="absolute top-10 sm:top-20 right-4 sm:right-10 w-16 sm:w-24 md:w-32 h-16 sm:h-24 md:h-32 border border-saffron/20 rounded-full"
          />
          <motion.div
            animate={{ 
              rotate: -360,
            ...
            className="absolute bottom-20 sm:bottom-40 left-4 sm:left-20 w-12 sm:w-18 md:w-24 h-12 sm:h-18 md:h-24 border border-saffron/20 transform rotate-45"
          />
          <motion.div
            animate={{ 
              scale: [1, 1.2, 1],
            ...
            className="absolute top-1/2 left-1/4 w-24 sm:w-32 md:w-48 h-24 sm:h-32 md:h-48 bg-gradient-to-br from-jet-stream-400/10 to-transparent rounded-full blur-xl"
          />
        </motion.div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          >
            <Trophy className="w-5 sm:w-6 md:w-8 h-5 sm:h-6 md:h-8 text-saffron" />
          </motion.div>
          <h2 className="font-[var(--font-display)] text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-3 sm:mb-4 md:mb-6 leading-tight px-2">
            Community Leaders &{" "}
            <span className="bg-gradient-to-r from-saffron via-accent to-jet-stream-300 bg-clip-text text-transparent">
              SkillCredits
            </span>
        ...
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 md:mb-8 px-1">
              <motion.div
                animate={{ scale: [1, 1.1, 1] }}
                transition={{ duration: 2, repeat: Infinity }}
                className="relative"
              >
                <Crown className="w-5 sm:w-6 md:w-8 h-5 sm:h-6 md:h-8 text-saffron drop-shadow-lg" />
                <motion.div
                  animate={{ opacity: [0, 1, 0] }}
                  transition={{ duration: 1.5, repeat: Infinity }}
                  className="absolute -inset-1 bg-saffron/20 rounded-full blur-sm"
              ...
              <h3 className="font-[var(--font-display)] text-xl sm:text-2xl md:text-3xl font-bold text-white">
                Community Leaders
              </h3>
            </div>
              ...
              {leaderboardData.slice(0, visibleItems).map((user, index) => (
                ...
                >
                  <Card className={`
                    bg-gradient-to-r from-jet-stream-900 via-jet-stream-900/95 to-jet-stream-900/90 
                    border-2 transition-all duration-500 group cursor-pointer overflow-hidden
                    ${user.rank === 1 ? 'border-saffron/40 hover:border-saffron/60 hover:shadow-lg sm:hover:shadow-xl lg:hover:shadow-2xl hover:shadow-saffron/20' : 
                      user.rank === 2 ? 'border-jet-stream-300/40 hover:border-jet-stream-300/60 hover:shadow-lg sm:hover:shadow-xl hover:shadow-jet-stream-300/20' :
                      user.rank === 3 ? 'border-accent/40 hover:border-accent/60 hover:shadow-lg sm:hover:shadow-xl hover:shadow-accent/20' :
                      'border-jet-stream-400/20 hover:border-jet-stream-400/40 hover:shadow-md sm:hover:shadow-lg hover:shadow-jet-stream-400/10'}
                  `}>
                    {/* Rank indicator line */}
                    <div className={`absolute top-0 left-0 w-full h-0.5 sm:h-1 ${
                      user.rank === 1 ? 'bg-gradient-to-r from-saffron to-saffron/60' :
                      user.rank === 2 ? 'bg-gradient-to-r from-jet-stream-300 to-jet-stream-300/60' :
                      user.rank === 3 ? 'bg-gradient-to-r from-accent to-accent/60' :
                      'bg-gradient-to-r from-jet-stream-400 to-jet-stream-400/60'
                    ...
                    <CardContent className="p-2.5 sm:p-3 md:p-4">
                      <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                        <motion.div 
                          ...
                          className="flex items-center justify-center w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 rounded-md sm:rounded-lg bg-jet-stream-950 border-2 border-jet-stream-400/30 group-hover:border-jet-stream-400/60 transition-all duration-300 flex-shrink-0"
                        >
                          {getRankIcon(user.rank)}
                        </motion.div>
                        ...
                        <motion.div 
                          whileHover={{ scale: 1.05 }}
                          className="w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 rounded-md sm:rounded-lg overflow-hidden bg-jet-stream-950 border-2 border-jet-stream-400/30 group-hover:border-jet-stream-400/60 transition-all duration-300 flex-shrink-0"
                        >
                          <img 
                            src={user.avatar} 
                            alt={user.name}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1 sm:gap-2 mb-1">
                            <h4 className="font-[var(--font-display)] font-semibold text-white text-xs sm:text-sm md:text-base truncate">
                              {user.name}
                            </h4>
                            <motion.div
                              animate={{ scale: user.trend === 'up' ? [1, 1.2, 1] : 1 }}
                          ...
                          <div className="flex flex-wrap gap-1 mb-1.5 sm:mb-2">
                            {user.skills.slice(0, isMobile ? 1 : 2).map((skill, skillIndex) => (
                              <Badge
                                key={skillIndex}
                                variant="outline"
                                className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0 bg-jet-stream-950/50 text-gray-300 border-jet-stream-400/30 hover:border-jet-stream-400/60 transition-colors duration-950"
                              >
                                {skill}
                              </Badge>
                            ))}
                          ...
                          <Badge className={`text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 ${getLevelBadgeColor(user.level)} block w-fit mb-1.5 sm:mb-2`}>
                            {user.level}
                          </Badge>

                          <div className="flex items-center gap-2 sm:gap-3 text-[10px] sm:text-xs text-gray-400">
                            <span className="truncate">{user.completedSessions} sessions</span>
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          >
                            ...
                            <span className="font-[var(--font-display)] text-xs sm:text-sm md:text-base">
                              {user.credits.toLocaleString()}
                            </span>
                          </motion.div>
                          <div className="text-[10px] sm:text-xs text-gray-400 mt-1">credits</div>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
            ...
            {visibleItems < 8 && (
              ...
              >
                <div className="text-xs sm:text-sm text-gray-400 mb-2">Loading more leaders...</div>
                <Progress 
                  value={(visibleItems / 8) * 100} 
                  className="w-24 sm:w-32 mx-auto h-1.5 sm:h-2"
                />
              </motion.div>
            )}
          </motion.div>
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 md:mb-8 px-1">
              <motion.div
                ...
                className="relative"
              >
                <Coins className="w-5 sm:w-6 md:w-8 h-5 sm:h-6 md:h-8 text-accent drop-shadow-lg" />
                <motion.div
                  animate={{ opacity: [0, 0.5, 0] }}
                  transition={{ duration: 2, repeat: Infinity }}
                  className="absolute -inset-2 bg-accent/20 rounded-full blur-md"
            ...
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 300 }}
            >
              <Card className="bg-gradient-to-br from-jet-stream-900 via-jet-stream-900/95 to-jet-stream-800/90 border-2 border-jet-stream-400/20 hover:border-saffron/40 p-3 sm:p-4 md:p-6 mb-4 sm:mb-6 md:mb-8 transition-all duration-500 hover:shadow-lg sm:hover:shadow-xl lg:hover:shadow-2xl hover:shadow-saffron/20 relative overflow-hidden">
                {/* Animated background glow */}
                <motion.div
                  animate={{ opacity: [0.1, 0.3, 0.1] }}
                  transition={{ duration: 4, repeat: Infinity }}
                ...
                <CardContent className="p-0 relative z-10">
                  <div className="text-center mb-3 sm:mb-4 md:mb-6">
                    <motion.div
                      ...
                      className="inline-flex items-center justify-center w-12 sm:w-14 md:w-16 h-12 sm:h-14 md:h-16 rounded-full bg-gradient-to-br from-saffron/20 via-saffron/30 to-saffron/20 border-2 border-saffron/40 mb-2 sm:mb-3 md:mb-4 shadow-lg shadow-saffron/30"
                    >
                      <Coins className="w-5 sm:w-6 md:w-8 h-5 sm:h-6 md:h-8 text-saffron" />
                    </motion.div>
                    <h4 className="font-[var(--font-display)] text-base sm:text-lg md:text-xl font-bold text-white mb-2">
                      Your Credit Balance
                    </h4>
                    <motion.div 
                      className="text-xl sm:text-2xl md:text-3xl font-bold text-saffron font-[var(--font-display)]"
                      animate={{ scale: currentBalance === 1250 ? [1, 1.05, 1] : 1 }}
                      transition={{ duration: 0.5 }}
                    >
                      {currentBalance.toLocaleString()} Credits
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    ...
                    <Progress value={68} className="h-1.5 sm:h-2 bg-jet-stream-800" />
                    <div className="text-[10px] sm:text-xs text-gray-400 text-center px-2">
                      580 more credits to reach Skill Master level
                    </div>
                  </div>
                </CardContent>
            ...
            <div className="grid gap-3 sm:gap-4 md:gap-6">
              {creditFeatures.map((feature, index) => (
                <motion.div
                  ...
                  className="group"
                >
                  <Card className="bg-gradient-to-r from-jet-stream-900 via-jet-stream-900/95 to-jet-stream-900/90 border-2 border-jet-stream-400/20 hover:border-jet-stream-400/40 transition-all duration-500 hover:shadow-lg sm:hover:shadow-xl hover:shadow-accent/10 relative overflow-hidden">
                    {/* Animated gradient background */}
                    <motion.div
                      animate={{ opacity: [0, 0.5, 0] }}
                      transition={{ duration: 3, repeat: Infinity, delay: index * 0.5 }}
                    ...
                    <CardContent className="p-3 sm:p-4 md:p-6 relative z-10">
                      <div className="flex items-start gap-2 sm:gap-3 md:gap-4">
                        <motion.div
                          ...
                          className="flex-shrink-0"
                        >
                          <div className="w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 rounded-md sm:rounded-lg bg-jet-stream-950 border-2 border-jet-stream-400/30 flex items-center justify-center group-hover:border-jet-stream-400/60 transition-all duration-300 shadow-lg">
                            <feature.icon className={`w-4 sm:w-5 md:w-6 h-4 sm:h-5 md:h-6 ${feature.color}`} />
                          </div>
                        </motion.div>
                        <div className="flex-1 min-w-0">
                          <h4 className="font-[var(--font-display)] text-sm sm:text-base md:text-lg font-semibold text-white mb-1 sm:mb-2">
                            {feature.title}
                          </h4>
                          <p className="text-gray-300 text-xs sm:text-sm leading-relaxed">
                            {feature.description}
            ...
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 300 }}
            >
              <Card className="bg-gradient-to-r from-saffron/10 via-accent/10 to-jet-stream-400/10 border-2 border-saffron/30 p-3 sm:p-4 md:p-6 relative overflow-hidden hover:shadow-lg sm:hover:shadow-xl hover:shadow-saffron/20 transition-all duration-500">
                <motion.div
                  animate={{ 
                    scale: [1, 1.5, 1],
                    opacity: [0.3, 0.6, 0.3] 
                  ...
                  className="absolute top-2 right-2"
                >
                  <Sparkles className="w-3 sm:w-4 md:w-6 h-3 sm:h-4 md:h-6 text-accent/60" />
                </motion.div>
                ...
                <CardContent className="p-0 text-center relative z-10">
                  <motion.div
                    ...
                    transition={{ duration: 4, repeat: Infinity }}
                  >
                    <Star className="w-5 sm:w-6 md:w-8 h-5 sm:h-6 md:h-8 text-saffron mx-auto mb-2 sm:mb-3" />
                  </motion.div>
                  <h4 className="font-[var(--font-display)] text-sm sm:text-base md:text-lg font-bold text-white mb-2">
                    Pro Tip
                  </h4>
        ...
        <motion.div
          ...
          transition={{ duration: 0.8, delay: 0.3 }}
          viewport={{ once: true }}
          className="mt-8 sm:mt-12 md:mt-16 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6"
        >
          {[
            { icon: Users, label: "Active Members", value: "12.5K+", color: "text-accent" },
            { icon: Target, label: "Skills Shared", value: "850+", color: "text-saffron" },
            ...
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 300 }}
            >
              <Card className="bg-gradient-to-br from-jet-stream-900/80 to-jet-stream-800/60 border-2 border-jet-stream-400/20 hover:border-jet-stream-400/40 p-3 sm:p-4 md:p-6 text-center group transition-all duration-500 hover:shadow-md sm:hover:shadow-lg hover:shadow-jet-stream-400/20">
                <motion.div
                  animate={{ scale: [1, 1.1, 1] }}
                  transition={{ duration: 2, repeat: Infinity, delay: index * 0.5 }}
                  className="mb-2 sm:mb-3"
                >
                  <stat.icon className={`w-5 sm:w-6 md:w-8 h-5 sm:h-6 md:h-8 ${stat.color} mx-auto`} />
                </motion.div>
                <div className={`text-lg sm:text-xl md:text-2xl font-bold ${stat.color} font-[var(--font-display)] mb-1`}>
                  {stat.value}
                </div>
                <div className="text-xs sm:text-sm text-gray-400 font-medium">
                  {stat.label}
