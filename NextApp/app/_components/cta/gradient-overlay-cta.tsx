...
//   return (
//     <section ref={ref} className="py-32 bg-jet-stream-400 relative overflow-hidden">
//       {/* Background Video */}
//       <video
...
const GradientOverlayCta = () => {
  ...
  // ✅ Fix: generate random positions client-side only
  const [shapes, setShapes] = useState<{ x: string; y: string }[]>([]);
  const [particles, setParticles] = useState<{ x: string; y: string }[]>([]);

  useEffect(() => {
    const newShapes = Array.from({ length: 6 }).map(() => ({
      x: `${Math.random() * 100}%`,
      y: `${Math.random() * 100}%`
    }));
  ...
  return (
    <section
      ref={ref}
      className="py-32 bg-jet-stream-400 relative overflow-hidden"
    >
      {/* Background Video */}
      <video
        className="absolute inset-0 h-full w-full object-cover opacity-99"
      ...
      >
        <source
          src="https://slelguoygbfzlpylpxfs.supabase.co/storage/v1/object/public/project-uploads/e4f0325f-5958-42aa-a841-f191ceb5b1cc/generated_videos/indian-urban-cityscape-at-golden-hour-wi-7b6adbdf-20250816125159.mp4?"
          type="video/mp4"
        />
      </video>
        ...
        >
          ...
          <div className="relative z-10 flex flex-col gap-12 p-4 text-center max-w-4xl">
            ...
            <motion.p
              ...
              viewport={{ once: true }}
            >
              Join thousands of Indians already earning and learning in their
              local communities
            </motion.p>
            ...
            >
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-jet-stream-200 font-body">
                  Active Users
                </span>
              </div>
              <div className="flex flex-col items-center gap-2">
